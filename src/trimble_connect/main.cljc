(ns trimble-connect.main
  "Kotodama WASM entrypoint for the Trimble_connect clean-room actor (L4) — Clojure port.
  ...
  facts (`trimble_connect.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "trimble_connect")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"trimble_connect.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "trimble_connect-compat" :tier tier :entities entities} 200])
...
(defn start! [] :trimble_connect-compat/ready)
